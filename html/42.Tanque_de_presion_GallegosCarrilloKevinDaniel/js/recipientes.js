function calcularepFcn() {
    ...
    Vtapas = Math.PI/12*(Math.pow(diametro + 2*espesortapareal,3) - Math.pow(diametro,3));
        ...
        /*Calculo para las cosas de las Tareas */
        /*Volumen de un liquido */

/* Cálculo del volumen del líquido */
var areaBase = Math.PI / 4 * Math.pow(diametro, 2); // Área de la base
var volumenLiquido = areaBase * nivel; // Volumen en m³
document.querySelector("#VolumenLiquido").value = volumenLiquido.toFixed(4); // Mostrar el volumen

/*Peso total del equipo o Peso LLeno */

var Rsuma = pesoequipo + (volumenLiquido * rho); // Corrige aquí usando 'rho'
document.querySelector("#pesoequipolleno").value = Rsuma.toFixed(4);
